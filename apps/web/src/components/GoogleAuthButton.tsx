"use client";

import { useEffect, useState } from "react";
import { useT } from "@/i18n/LocaleProvider";

type Props = {
  mode: "login" | "register";
};

export function GoogleAuthButton({ mode }: Props) {
  const t = useT();
  const [enabled, setEnabled] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/api/v1/auth/providers", { credentials: "include" })
      .then((r) => (r.ok ? r.json() : { google: false }))
      .then((data: { google?: boolean }) => {
        if (!cancelled) setEnabled(Boolean(data.google));
      })
      .catch(() => {
        if (!cancelled) setEnabled(false);
      });
    return () => {
      cancelled = true;
    };
  }, []);

  if (!enabled) return null;

  const label = mode === "login" ? t.continueWithGoogle : t.signUpWithGoogle;

  return (
    <>
      <a className="btn btn-google" href="/api/v1/auth/google/start">
        <GoogleGlyph />
        {label}
      </a>
      <div className="auth-divider" role="separator" aria-label={t.orContinueWith}>
        <span>{t.orContinueWith}</span>
      </div>
    </>
  );
}

function GoogleGlyph() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden>
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.2-.1-2.3-.4-3.5z"
      />
      <path
        fill="#FF3D00"
        d="M6.3 14.7l6.6 4.8C14.7 16 19 12 24 12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 16.3 4 9.6 8.3 6.3 14.7z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.3C29.3 35.3 26.8 36 24 36c-5.3 0-9.7-3.3-11.3-7.9l-6.5 5C9.5 39.6 16.2 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.3 4.1-4.1 5.5l.1.1 6.3 5.3C39.1 37.2 44 32 44 24c0-1.2-.1-2.3-.4-3.5z"
      />
    </svg>
  );
}
